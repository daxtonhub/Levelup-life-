...
async function loadAllData() {
  const [dq, sq, sk] = await Promise.all([
    sb.from('daily_quests').select('*').eq('user_id', user.id).eq('quest_date', today),
...
async function generateDailyQuests() {
  const { data: ex } = await sb.from('daily_quests').select('id').eq('user_id', user.id).eq('quest_date', today).limit(1);
  ...
  await sb.from('daily_quests').insert(DEFAULT_QUESTS.map(q => ({
    ...
    xp_reward: DIFF_CONFIG[q.diff].xp, quest_date: today, is_completed: false, difficulty: q.diff, is_recurring: q.type === 'habit'
  })));
...
async function generatePeriodQuests() {
  ...
  await sb.from('daily_quests').insert(pqs.map(q => ({ user_id: user.id, title: q.title, quest_type: 'habit', skill_category: 'mindset', xp_reward: q.xp_reward || DIFF_CONFIG.easy.xp, quest_date: today, is_completed: false, difficulty: 'easy', is_recurring: true })));
}
...
function checkDailyBonusAvailable() {
  ...
  if (done === dailyQuests.length && profile?.daily_bonus_claimed !== today) show('bonus-banner-wrap');
...
window.claimDailyBonus = async () => {
  ...
  profile.daily_bonus_claimed = today; profile.summon_shards = newShards;
...
async function renderQuestsTab() {
  ...
  else if (qFilter === 'scheduled')   { const { data: future } = await sb.from('daily_quests').select('*').eq('user_id', user.id).gt('quest_date', today).order('quest_date', { ascending: true }).limit(20); list = future || []; }
...
async function checkStreak() {
  ...
  if (profile.streak_shields > 0 && profile.last_active !== yesterday && profile.last_active !== today) { newStreak = profile.streak || 0; await sb.from('profiles').update({ streak_shields: (profile.streak_shields||1) - 1 }).eq('id', user.id); profile.streak_shields = Math.max(0, (profile.streak_shields||1) - 1); toast('🛡️ Streak Shield activated!', 'green'); }
...
// ─── ADD QUEST ────────────────────────────────
window.openAddQuest = () => { const d = id('nq-date'); if(d) d.value = today; show('add-quest-modal'); };

window.addQuest = async () => {
  const title = val('nq-title'), type = val('nq-type'), diff = val('nq-difficulty') || 'medium', skill = val('nq-skill');
  const schedDate = val('nq-date') || today, schedTime = document.getElementById('nq-time')?.value || null;
  ...
  if (data && schedDate === today) dailyQuests.push(data);
...
window.executeNameChange = async () => {
  ...
  await sb.from('profiles').update({ username: newName, coins: nc, name_change_count: count + 1, name_change_reset_date: today }).eq('id', user.id);
...
async function checkLoginReward() {
  ...
  profile.last_login_date = today; profile.login_streak_count = loginStreak;
  ...
  el.innerHTML = `
    <div style="text-align:center;padding:16px 0">
      ...
      <div class="login-reward-days">
        ${[1,2,3,4,5,6,7].map(d => `<div class="lr-day ${d<day?'claimed':''} ${d===day?'today':''}"><div class="lr-day-num">D${d}</div><div class="lr-day-icon">${d===7?'🎫':d===3||d===6?'🔮':'🎁'}</div></div>`).join('')}
      </div>
...
async function exportAccountData() {
  ...
  a2.href = url; a2.download = `levelup-backup-${today}.json`; a2.click();
...
async function saveOnboarding() {
  ...
  if (obGeneratedQuests.length > 0) {
    await sb.from('daily_quests').insert(obGeneratedQuests.map(q => {
      ...
      return { user_id: user.id, title: q.title, quest_type: 'habit', skill_category: 'discipline', xp_reward: dc.xp, quest_date: today, is_completed: false, difficulty: diff, is_recurring: true };
    }));
  }
  if (obStarterPet) {
