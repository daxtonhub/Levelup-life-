...
window.saveJournalEntry = async () => {
  const title = val('je-title') || 'Entry ' + today, content = document.getElementById('je-content').value.trim();
...
window.attackBoss = async (bossId, xpPerHit, currentHp, totalXpRew, coinsRew) => {
  ...
  try {
    const { data: ex } = await sb.from('boss_participants').select('*').eq('boss_id', bossId).eq('user_id', user.id).maybeSingle();
...
async function startComebackSession() {
  try {
    ...
    await sb.from('daily_quests').insert([
      { user_id: user.id, title: 'Complete 1 small task right now', quest_type: 'todo', skill_category: 'mindset', xp_reward: 50, quest_date: today, is_completed: false, difficulty: 'easy' },
      { user_id: user.id, title: 'Drink a glass of water', quest_type: 'todo', skill_category: 'discipline', xp_reward: 50, quest_date: today, is_completed: false, difficulty: 'easy' },
      { user_id: user.id, title: 'Write one sentence in your journal', quest_type: 'todo', skill_category: 'mindset', xp_reward: 50, quest_date: today, is_completed: false, difficulty: 'easy' }
    ]);
...
window.createBoss = async () => {
  ...
  await sb.from('boss_fights').insert({ title, description: desc, boss_icon: icon, total_hp: hp, current_hp: hp, xp_per_hit: xpHit, xp_reward: xpRew, coins_reward: coinsRew, end_date: endDate, start_date: today, is_active: true, is_defeated: false });
...
window.createEvent = async () => {
  ...
  const startDate = val('ev-start') || today, endDate = val('ev-end') || null;
...
window.viewPlayer = async (userId) => {
  ...
  const todayQ = (quests||[]).filter(q => q.quest_date === today);
