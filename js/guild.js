...
async function renderAdmin() {
  var isAdmin = profile && profile.is_admin || user && user.email && user.email.toLowerCase() === ADMIN_EMAIL.toLowerCase();
  ...
  var users   = (await sb.from('profiles').select('*')).data;
  var squests = (await sb.from('system_quests').select('*')).data;
  var comps   = (await sb.from('user_system_quests').select('*').eq('is_completed', true)).data;
  var totalXP = (users || []).reduce(function(a, u){ return a + (u.xp || 0); }, 0);
  var assignOptions = '<option value="">All Players</option>';
  (users || []).forEach(function(u){ assignOptions += '<option value="' + u.id + '">' + (u.username || 'Unknown') + '</option>'; });
  var sqHtml = (squests || []).length === 0 ? empty('📋', 'No system quests yet') : '';
  (squests || []).forEach(function(q){ sqHtml += adminQCard(q, users); });
  var playersHtml = (users || []).length === 0 ? empty('👤', 'No players yet') : '';
  var sortedUsers = [...(users || [])].sort(function(a, b){ return (b.xp || 0) - (a.xp || 0); });
  sortedUsers.forEach(function(u, i) {
    var role = getRoleInfo(u.role || 'warrior');
    playersHtml += '<div class="player-card" onclick="viewPlayer(\'' + u.id + '\')"><div class="player-card-top"><div class="player-av">' + (u.avatar_url ? '<img src="' + u.avatar_url + '"/>' : '🧙') + '</div><div style="flex:1"><div class="player-name">' + (i === 0 ? '🥇 ' : '') + (u.username || 'Unknown') + (u.is_admin ? ' 👑' : '') + (u.gender === 'female' ? ' 🌸' : '') + '</div><div class="player-meta">' + role.icon + ' ' + role.name + ' • 🔥 ' + (u.streak || 0) + ' • 🪙' + (u.coins || 0) + '</div></div><div class="player-xp">' + (u.xp || 0) + ' XP</div></div><div class="player-mini-stats"><div class="mini-stat"><div class="mini-val">' + (u.xp || 0) + '</div><div class="mini-lbl">XP</div></div><div class="mini-stat"><div class="mini-val">' + getLevel(u.xp || 0) + '</div><div class="mini-lbl">LV</div></div><div class="mini-stat"><div class="mini-val">' + (u.streak || 0) + '</div><div class="mini-lbl">STREAK</div></div><div class="mini-stat"><div class="mini-val" style="color:var(--coin)">' + (u.coins || 0) + '</div><div class="mini-lbl">COINS</div></div></div><div class="view-profile-btn">View Full Profile →</div></div>';
  });
  var html = '<div class="admin-banner"><div class="admin-crown">👑</div><div><div class="admin-banner-title">HOST CONTROL PANEL</div><div class="admin-banner-sub">Welcome, ' + profile.username + '</div></div></div>';
  html += '<div class="admin-stats"><div class="astat"><div class="astat-val">' + (users || []).length + '</div><div class="astat-lbl">PLAYERS</div></div><div class="astat"><div class="astat-val">' + (squests || []).length + '</div><div class="astat-lbl">QUESTS</div></div><div class="astat"><div class="astat-val">' + (comps || []).length + '</div><div class="astat-lbl">DONE</div></div><div class="astat"><div class="astat-val">' + totalXP + '</div><div class="astat-lbl">TOTAL XP</div></div></div>';
  html += '<div class="admin-tabs"><button class="admin-tab active" onclick="switchAdminTab(\'quests\',this)">📋 Quests</button><button class="admin-tab" onclick="switchAdminTab(\'players\',this)">👥 Players</button><button class="admin-tab" onclick="switchAdminTab(\'bosses\',this)">🐉 Bosses</button><button class="admin-tab" onclick="switchAdminTab(\'events\',this)">🎪 Events</button><button class="admin-tab" onclick="switchAdminTab(\'vip\',this)">⭐ VIP</button><button class="admin-tab" onclick="switchAdminTab(\'summons\',this)">🔮 Summons</button></div>';
  html += '<div class="admin-section active" id="asec-quests"><div class="form-card"><div class="form-card-title">Create System Quest</div><div class="form-group"><label>Title</label><input type="text" id="sq-title" placeholder="Quest title"/></div><div class="form-group"><label>Description</label><textarea id="sq-desc" placeholder="Quest description..."></textarea></div><div class="form-group"><label>XP Reward</label><input type="number" id="sq-xp" value="50" min="1"/></div><div class="form-group"><label>Type</label><select id="sq-type"><option value="challenge">Challenge</option><option value="habit">Habit (daily)</option><option value="todo">Task (one-time)</option></select></div><div class="form-group"><label>Skill</label><select id="sq-skill-cat"><option value="fitness">Fitness</option><option value="mindset">Mindset</option><option value="knowledge">Knowledge</option><option value="discipline">Discipline</option><option value="creativity">Creativity</option><option value="social">Social</option></select></div><div class="form-group"><label>Assign To</label><select id="sq-assign">' + assignOptions + '</select></div><button class="btn-gold" onclick="createSQ()">CREATE QUEST</button></div><div class="sec-title">Active System Quests</div><div id="admin-sq-list">' + sqHtml + '</div></div>';
  html += '<div class="admin-section" id="asec-players"><div class="sec-title">All Players</div>' + playersHtml + '</div>';
  html += '<div class="admin-section" id="asec-bosses"><div class="form-card"><div class="form-card-title">Create Boss Fight</div><div class="form-group"><label>Boss Name</label><input type="text" id="boss-name" placeholder="e.g. The Procrastination Dragon"/></div><div class="form-group"><label>Description</label><textarea id="boss-desc" placeholder="What must players overcome?"></textarea></div><div class="form-group"><label>Boss Icon</label><input type="text" id="boss-icon" placeholder="🐉" maxlength="2"/></div><div class="form-group"><label>Total HP</label><input type="number" id="boss-hp" value="1000" min="100"/></div><div class="form-group"><label>XP Per Hit</label><input type="number" id="boss-xp-hit" value="10" min="1"/></div><div class="form-group"><label>Victory XP Reward</label><input type="number" id="boss-xp-reward" value="500" min="50"/></div><div class="form-group"><label>Victory Coins Reward</label><input type="number" id="boss-coins-reward" value="200" min="10"/></div><div class="form-group"><label>End Date</label><input type="date" id="boss-end-date"/></div><button class="btn-gold" onclick="createBoss()">CREATE BOSS</button></div></div>';
  html += '<div class="admin-section" id="asec-events"><div class="form-card"><div class="form-card-title">Create Event</div><div class="form-group"><label>Event Title</label><input type="
