...
// ── AI via Supabase Edge Function (key stays secure) ──
async function callAI(messages, system = '', maxTokens = 600) {
...
function val(i)   { const el = document.getElementById(i); return el ? el.value.trim() : ''; }
function show(i)  { const el = document.getElementById(i); if (el) el.classList.remove('hidden'); }
...
function toast(msg, type = '') {
  ...
  t.id = '_toast'; t.className = 'toast ' + type; t.textContent = msg;
...
window.closeModal    = (mid) => hide(mid);
window.closeModalBg  = (e, mid) => { if (e.target.id === mid) hide(mid); };
